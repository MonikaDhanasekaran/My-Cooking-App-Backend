...
const chineseRecipe = require("../models/chineseRecipe");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        chineseRecipe.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new chineseRecipe(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        chineseRecipe.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        chineseRecipe.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
