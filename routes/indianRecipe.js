...
const indianRecipe = require("../models/indianRecipe");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        indianRecipe.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new indianRecipe(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        indianRecipe.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        indianRecipe.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
