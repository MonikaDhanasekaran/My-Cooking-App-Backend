...
const continentalRecipe = require("../models/continentalRecipe");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        continentalRecipe.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new continentalRecipe(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        continentalRecipe.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        continentalRecipe.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
