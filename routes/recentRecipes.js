...
const recentRecipes = require("../models/recentRecipes");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        recentRecipes.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new recentRecipes(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        recentRecipes.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        recentRecipes.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
