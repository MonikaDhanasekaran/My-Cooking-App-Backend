...
const snackRecipe = require("../models/snackRecipe");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        snackRecipe.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new snackRecipe(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        snackRecipe.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        snackRecipe.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
