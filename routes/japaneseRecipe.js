...
const japaneseRecipe = require("../models/japaneseRecipe");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        japaneseRecipe.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new japaneseRecipe(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        japaneseRecipe.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        japaneseRecipe.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
