...
const starterRecipe = require("../models/starterRecipe");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        starterRecipe.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new starterRecipe(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        starterRecipe.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        starterRecipe.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
